...
import {useLocaleIso} from "~/utils";
...
export const useCategoryStore = defineStore('category', () => {
     ...
     * 获取分类
     */
    async function getCategory(id: number, fieldMask?: undefined | string) {
        const locale = useLocaleIso();
    ...
    function getTranslation(category: contentservicev1_Category) {
        ...
        const locale = useLocaleIso();
        // 优先查找当前语言的翻译
        const translation = category.translations?.find((t: contentservicev1_CategoryTranslation) => t.languageCode === locale.value);
        // 如果找不到，回退到第一个翻译
