...
└──────────────┬──────────────────────────┘
               │ 使用 Composables
┌──────────────▼──────────────────────────┐
│      API Composables Layer (React Query层)     │
│  - useXxx() - React Composables               │
│  - fetchXxx() - 非 Hook 方法             │
└──────────────┬──────────────────────────┘
               │ 调用 Service
┌──────────────▼──────────────────────────┐
...
│   └── ...
│
├── composables/                    # Composables 层 - Vue Query 集成
│   ├── auth.ts                   # 认证相关 Composables
│   ├── user.ts                   # 用户管理 Composables
│   ├── index.ts                  # 统一导出
│   └── ...
│
└── index.ts                      # API 模块总入口
...
- ❌ 不要在 Service 层使用 React Composables
- ❌ 不要直接暴露 Service Client 给 UI 层

---

### 2️⃣ Composables 层（React Query 集成层）

**位置**: `src/api/composables/*.ts`

**职责**:

- 将 Service 层的函数包装为 React Composables
...
// ==============================
// React Composables（用于 React 组件中）
// ==============================
...
- ❌ 不要在 Composables 层直接创建 Service Client
- ❌ 不要在 Composables 层直接使用 `requestApi`
...
```tsx
import { useListUsers, useGetUser, useCreateUser } from '@/api/composables/user';
...
import { fetchLogin, fetchUserProfile } from '@/api/composables/auth';
import { fetchListUsers } from '@/api/composables/user';
...
import { fetchNavigation, fetchMyPermissionCode } from '@/api/composables/app-portal';
...
```typescript
// utils/permission.ts
import { fetchMyPermissionCode } from '@/api/composables/app-portal';
...
// ✅ Composables 层：通过 React Query 的 onError 处理
...
### Q1: 为什么需要 Service 层和 Composables 层？

**A**:

- **Service 层**：封装纯函数式 API 调用，可在任何环境中使用（React、Node.js、测试等）
- **Composables 层**：集成 React Query，提供声明式数据获取、缓存、重试等功能
...
3. **创建 Composables 文件** (`src/api/composables/xxx.ts`)
   ```typescript
...
4. **导出模块** (`src/api/service/index.ts` 和 `src/api/composables/index.ts`)
   ```typescript
   export * from './xxx';
   ```
