...
-- ============================================================================
-- 内容归属测试租户(id=1):tenant_id=0 不进搜索索引且被租户过滤
-- ============================================================================
...
-- 开发环境:匿名租户解析按 Host 匹配租户 domain(带端口会去端口回退匹配)
...
INSERT IGNORE INTO sys_user_roles (tenant_id, user_id, role_id)
SELECT 1, 2, r.id FROM sys_roles r WHERE r.code = 'template:tenant:manager' AND r.tenant_id = 1;

-- ============================================================================
-- C端访问权限 + 租户普通用户角色(注册默认角色,不再授租户管理员)
-- ============================================================================
INSERT IGNORE INTO sys_permissions (code, name, status, tenant_id, description)
VALUES ('sys:access_app', '访问应用端', 'ON', 0, 'C端(应用)登录访问权限');

INSERT IGNORE INTO sys_roles (code, name, status, tenant_id)
VALUES ('tenant:user', '普通用户', 'ON', 1);

INSERT IGNORE INTO sys_role_permissions (role_id, permission_id, tenant_id, status)
